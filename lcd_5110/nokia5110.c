...
void nokia_lcd_write_char(char code)
{
	...
	if (code != '\n')
	{
		...
		for (x = 0; x < 5; x++)
			for (y = 0; y < 7; y++)
				if (pgm_read_byte(&CHARSET[code-NOKIA_CHARS_ASCII_OFFSET][x]) & (1 << y))
					nokia_lcd_set_pixel(nokia_lcd.cursor_x + x, nokia_lcd.cursor_y + y, 1);
				else
					nokia_lcd_set_pixel(nokia_lcd.cursor_x + x, nokia_lcd.cursor_y + y, 0);

		nokia_lcd.cursor_x += 6;
	}
	else
	{
		//crlf
		nokia_lcd.cursor_x=84;
	}
	if (nokia_lcd.cursor_x >= 84) {
		nokia_lcd.cursor_x = 0;
		nokia_lcd.cursor_y += 8;
	}
...
void nokia_lcd_get_cursor(uint8_t* x, uint8_t* y)
{
	*x = nokia_lcd.cursor_x;
	*y = nokia_lcd.cursor_y;
}
